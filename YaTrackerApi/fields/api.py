...
from .categories import FieldCategoriesAPI
...
class FieldsAPI(BaseAPI):
    ...
    @property
    def categories(self) -> FieldCategoriesAPI:
        """Доступ к API для работы с категориями полей задач"""
        if self._categories is None:
            self._categories = FieldCategoriesAPI(self.client)
